...
class System(object):
    def __init__(self):
        ...
        self.at['part_solver'] = Boris_Push(self.at['pic'], [self.at['electrons'].name, self.at['electrons_HET'].name, self.at['photoelectrons'].name, self.at['see'].name, self.at['ions_HET'].name, self.at['protons'].name],\
                [self.at['electrons'].part_values.max_n, self.at['electrons_HET'].part_values.max_n, self.at['photoelectrons'].part_values.max_n, self.at['see'].part_values.max_n, self.at['ions_HET'].part_values.max_n, self.at['protons'].part_values.max_n],\
                [self.at['electrons'].vel_dim, self.at['electrons_HET'].vel_dim, self.at['photoelectrons'].vel_dim, self.at['see'].vel_dim, self.at['ions_HET'].vel_dim, self.at['protons'].vel_dim])

    def arrangePickle(self):
        #return ('ts', 'e_field', 'electrons', 'protons', 'user', 'part_solver')
        return ('ts', 'e_field', 'm_field', 'electrons', 'electrons_HET', 'photoelectrons', 'see', 'ions_HET', 'protons', 'part_solver')

    def arrangeVTK(self):
        #return ('ts', 'e_field', 'electrons', 'protons', 'user')
        return ('ts', 'e_field', 'm_field', 'electrons', 'electrons_HET', 'photoelectrons', 'see', 'ions_HET', 'protons')

    def arrangeParticlesTXT(self):
        #return ('ts', 'electrons', 'protons', 'user')
        return ('ts', 'electrons', 'electrons_HET', 'photoelectrons', 'see', 'ions_HET', 'protons')
...
#HET electrons
in_HET_e_n = c.HET_E_N*numpy.ones((len(system.at['mesh'].children[0].children[0].boundaries[2].exit_nodes)))
in_drift_HET_e_vel = numpy.zeros((len(system.at['mesh'].children[0].children[0].boundaries[2].exit_nodes),c.DIM))
in_thermal_HET_e_vel = c.HET_E_V_TH_MP*numpy.ones((len(system.at['mesh'].children[0].children[0].boundaries[2].exit_nodes)))
in_drift_HET_e_vel[:,1] += c.HET_E_V_DRIFT
#Photoelectrons
...
#HET ions
in_HET_ion_n = c.HET_ION_N*numpy.ones((len(system.at['mesh'].children[0].children[0].boundaries[2].exit_nodes)))
in_drift_HET_ion_vel = numpy.zeros((len(system.at['mesh'].children[0].children[0].boundaries[2].exit_nodes),c.DIM))
in_thermal_HET_ion_vel = c.HET_ION_V_TH_MP*numpy.ones((len(system.at['mesh'].children[0].children[0].boundaries[2].exit_nodes)))
in_drift_HET_ion_vel[:,1] += c.HET_ION_V_DRIFT
#Protons
...
def HET_E():
    ...
    #Injection of HET electrons at the exit of the thruster
    flux, new_HET_e = system.at['mesh'].children[0].children[0].boundaries[2].createDistributionHET(system.at['part_solver'],\
                                                               system.at['electrons_HET'], in_HET_e_n, in_drift_HET_e_vel, system.at['mesh'].children[0].children[0].start_ind)
    system.at['mesh'].children[0].children[0].boundaries[2].injectParticlesAtPositions_smooth(flux, system.at['part_solver'],\
                                                               system.at['e_field'], system.at['electrons_HET'], new_HET_e, in_thermal_HET_e_vel, in_drift_HET_e_vel, system.at['electrons_HET'].dt)
...
def HET_ION():
    ...
    if system.at['ts']%c.VTK_TS == 0:
        system.at['part_solver'].advance(system.at['ions_HET'], [system.at['e_field']], [system.at['m_field']], extent = 1)
    else:
        system.at['part_solver'].advance(system.at['ions_HET'], [system.at['e_field']], [system.at['m_field']], extent = 1, update_dic = 0)
    #Injection of HET ions at the exit of the thruster
    flux, new_HET_ion = system.at['mesh'].children[0].children[0].boundaries[2].createDistributionHET(system.at['part_solver'],\
                                                               system.at['ions_HET'], in_HET_ion_n, in_drift_HET_ion_vel, system.at['mesh'].children[0].children[0].start_ind)
    system.at['mesh'].children[0].children[0].boundaries[2].injectParticlesAtPositions_smooth(flux, system.at['part_solver'],\
                                                               system.at['e_field'], system.at['ions_HET'], new_HET_ion, in_thermal_HET_ion_vel, in_drift_HET_ion_vel, system.at['ions_HET'].dt)
...
try:
    while system.at['ts'] < c.NUM_TS:
        ...
        # Ions motion
        if system.at['ts']%c.HET_ION_TS == 0:
            HET_ION()
