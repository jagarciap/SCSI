...
class Inner_2D_Rectangular(Boundary):
    ...
    def applyParticleBoundary(self, species, type_boundary, albedo = None, **kwargs):
        ...
        out_ind = af.leq_2D_p(species.part_values.position[:np,:], xmin, xmax, ymin, ymax)
        ...
        if type_boundary == 'mixed':
            ...
            return self.applyParticleReflectiveBoundary(species, out_ind, kwargs['old_species'])
        else:
...
#               The tuple (pos, border) is reffered as flux in the program.
    ...
    def createDistributionAtBorder(self, location, part_solver, species, delta_n, prec = 1e-5):
        ...
        #Treating borders
        mpf_new /= numpy.where(numpy.max(part_solver.pic.mesh.volumes)/part_solver.pic.mesh.volumes[location] > 3, 2, 1)
