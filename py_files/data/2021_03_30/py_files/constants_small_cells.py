...
P_DT= 4e-8#5e-8            # time step size
E_DT = 2e-9#5e-8        # time step for electron dynamics
ELECTRON_TS = 20   #Internal iterations for electron dynamics
...
EPS_SAT_R =3.3378   # F/m, satellite material permittivity
...
PHE_N = PHE_FLUX/(PHE_V_TH_MP/numpy.sqrt(numpy.pi))/(-QE)     #This would be the density in space for FLUX and T.
PHE_N_WALL = PHE_FLUX/(-QE)*E_DT/DX   #This is the numeric density created in a mesh cell after one electron timestep

SEE_T = 2*EV_TO_K
SEE_V_TH = numpy.sqrt(2*K*SEE_T/ME)
...
P_SIZE = numpy.uint32(1.5e7)     #Size of the ions array
E_SIZE = numpy.uint32(1.5e7)     #Size of the electrons array
PHE_SIZE = numpy.uint32(1.5e6)     #Size of the electrons array
SEE_SIZE = numpy.uint32(1.5e6)     #Size of the electrons array
P_SPWT = 2e5
E_SPWT = 2e5
PHE_SPWT = 2e5
SEE_SPWT = 2e5
