...
#       +saveVTK(string filename, dict dictionary) = It calls the appropiate method in 'vtk' module to print the information of the system in a '.vtk' file.
    def saveVTK(self, filename, dictionary):
        i = numpy.arange(self.xmin, self.xmax+self.dx, self.dx, dtype ='single')
        j = numpy.arange(self.ymin, self.ymax+self.dy, self.dy, dtype ='single')
...
class Mesh_2D_rm_sat (Mesh_2D_rm):
    ...
    def setDomain(self):
        ...
        self.boundaries[1].directions[0] = 3
        ...
        self.direction_sat = numpy.zeros((0), dtype ='float32')
        self.adjacent_sat = []
...
#    def checkPositionInMesh(self, pos, surface = False):
#        xminsat = self.xminsat
#        xmaxsat = self.xmaxsat
#        yminsat = self.yminsat
#        ymaxsat = self.ymaxsat
#        #Outer boundary
#        mask1 = super().checkPositionInMesh(pos, surface = surface)
#        #Inner boundary
#        if surface and self.boundaries[1].material != "space":
#            mask2 = af.not_gl_p(pos, xminsat, xmaxsat, yminsat, ymaxsat)
#        else:
#            mask2 = af.not_gleq_p(pos, xminsat, xmaxsat, yminsat, ymaxsat)
#
#        return numpy.logical_and(mask1, mask2)
    ...
    def loadSpeciesVTK(self, species, pic):
        ...
        #Volume
        dv = self.dx*self.dy*self.depth
        particles = (species.mesh_values.density*dv/species.spwt).astype(int)
        ind = numpy.arange(self.nPoints)
        index = numpy.repeat(ind, particles)
        #Setting up positions
        pos = self.getPosition(index)
