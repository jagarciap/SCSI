...
class System(object):
    def __init__(self):
        ...
        self.at['part_solver'] = Boris_Push(self.at['pic'], [self.at['electrons'].name, self.at['see'].name, self.at['protons'].name],\
                [self.at['electrons'].part_values.max_n, self.at['see'].part_values.max_n, self.at['protons'].part_values.max_n],\
                [self.at['electrons'].vel_dim, self.at['see'].vel_dim, self.at['protons'].vel_dim])

    def arrangePickle(self):
        #return ('ts', 'e_field', 'electrons', 'protons', 'user', 'part_solver')
        return ('ts', 'e_field', 'm_field', 'electrons', 'see', 'protons', 'part_solver')

    def arrangeVTK(self):
        #return ('ts', 'e_field', 'electrons', 'protons', 'user')
        return ('ts', 'e_field', 'm_field', 'electrons', 'see', 'protons')

    def arrangeParticlesTXT(self):
        #return ('ts', 'electrons', 'protons', 'user')
        return ('ts', 'electrons', 'see', 'protons')
...
for i in [3]:
    ...
    p_n_mod = p_n[i]*(thermal_p_vel[i]/2/1.7724538509055159+drift_p_vel[i][:,0])*system.at['protons'].dt
    flux, n_delta_p = system.at['mesh'].boundaries[i].createDistributionAtBorder(system.at['mesh'].boundaries[i].location, system.at['part_solver'], system.at['protons'], p_n_mod)
    system.at['mesh'].boundaries[i].injectParticlesAtPositions_smooth(flux, system.at['part_solver'], \
                                                               system.at['e_field'], system.at['protons'], n_delta_p, thermal_p_vel[i], drift_p_vel[i], system.at['protons'].dt)
    e_n_mod = e_n[i]*(thermal_e_vel[i]/2/1.7724538509055159+drift_e_vel[i][:,0])*system.at['electrons'].dt
    flux, n_delta_e = system.at['mesh'].boundaries[i].createDistributionAtBorder(system.at['mesh'].boundaries[i].location, system.at['part_solver'], system.at['electrons'], e_n_mod)
    system.at['mesh'].boundaries[i].injectParticlesAtPositions_smooth(flux, system.at['part_solver'], \
                                                               system.at['e_field'], system.at['electrons'], n_delta_e, thermal_e_vel[i], drift_e_vel[i], system.at['electrons'].dt)
...
try:
    while system.at['ts'] < c.NUM_TS:
        ...
        for te in range(c.ELECTRON_TS):
            print('te = ', te)
            system.at['e_field'].computeField([system.at['protons'], system.at['electrons'], system.at['see']])
            ...
            if te == 0:
                ...
                system.at['part_solver'].advance(system.at['see'], [system.at['e_field']], [system.at['m_field']], extent = 1, types_boundary = ['reflective', 'open', 'reflective', 'open'])
            else:
                advance_dict_e = system.at['part_solver'].advance(system.at['electrons'], [system.at['e_field']], [system.at['m_field']],\
                ...
                system.at['part_solver'].advance(system.at['see'], [system.at['e_field']], [system.at['m_field']], extent = 1, update_dic = 0, types_boundary = ['reflective', 'open', 'reflective', 'open'])
            ...
            system.at['mesh'].boundaries[1].injectParticlesAtPositions_smooth(advance_dict_e['flux'], system.at['part_solver'],\
                                                                       system.at['e_field'], system.at['see'], new_see, in_thermal_see_vel, in_drift_see_vel, system.at['see'].dt)
            ##NOTE: For testing
        ...
        if system.at['ts']%10000 == 0:
            out.particleTracker(old_system.at['ts'], old_system.at['protons'], old_system.at['electrons'], old_system.at['see'])
