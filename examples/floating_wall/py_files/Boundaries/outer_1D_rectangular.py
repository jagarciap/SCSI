...
#Methods:
#	+Boundary methods.
class Outer_1D_Rectangular(Boundary):
    type = "Outer - 1D_Rectangular"
...
#           Example: if the boundary is a right boundary, the cases where pos_x > xmax, but also ymax > pos_y or ymin < pos_y.
    def checkPositionInBoundary(self, pos, surface = False, prec = 10**(-c.INDEX_PREC)):
        if self.directions[0] == 0:
            diff = pos[:,1]-self.ymin
        elif self.directions[0] == 1:
            diff = self.xmax-pos[:,0]
        elif self.directions[0] == 2:
            diff = self.ymax-pos[:,1]
        elif self.directions[0] == 3:
            diff = pos[:,0]-self.xmin
...
#       +createDummyBox([ind]location, PIC pic, Species species, [double] delta_n, [double] n_vel, [double] shift_vel) = create the dummy boxes with particles in them.
    def createDummyBox(self, location, pic, species, delta_n, n_vel, shift_vel, prec = 10**(-c.INDEX_PREC)):
        ...
        random += numpy.where(random == 0, prec, 0)
        #Bottom
        if self.directions[0] == 0:
...
#               The tuple (pos, border) is reffered as flux in the program.
    @Timing
    def createDistributionAtBorder(self, location, part_solver, species, delta_n, prec = 10**(-c.INDEX_PREC)):
...
#           according to the method used 'part_solver' for advancing particles.
    @Timing
    def injectParticlesAtPositions_smooth(self, hit, part_solver, field, species, delta_n, n_vel, drift_vel, dt, delta_pos = 10**(-c.INDEX_PREC)):
